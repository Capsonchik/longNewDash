...
export const Map = ({valueSetter}: Props) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const currentPath = pathsData[currentIndex];
      setActivePath(currentPath.id); // Устанавливаем активный path
      setTooltipContent(currentPath.info); // Устанавливаем контент тултипа
      setTooltipVisible(true); // Показываем тултип

      // Через 3 секунды скрываем тултип и переходим к следующему path
      setTimeout(() => {
        setTooltipVisible(false); // Скрываем тултип
        currentIndex = (currentIndex + 1) % pathsData.length; // Переходим к следующему path
      }, 3000); // Время отображения тултипа
  ...
  return (
    <div className="rf-map margin-top-20">
      {regionName !== null ? <div className={"cityName"}>{regionName}</div> : null}
